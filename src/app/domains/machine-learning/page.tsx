...
import { Brain, ArrowLeft, Sun, Moon, Database, Cpu, Network, Sparkles, Code2, Layers } from "lucide-react";
import { ParticleBackground } from "@/components/ParticleBackground";
import { Footer } from "@/components/Footer";

export default function MachineLearningDomain() {
  ...
  const topics = [
    {
      ...
      description: "Deep learning architectures and backpropagation logic.",
      icon: Network,
      color: "from-fuchsia-500 to-purple-600"
    },
    {
      title: "Data Engineering",
      description: "Pipeline optimization and feature extraction strategies.",
      icon: Database,
      color: "from-blue-500 to-indigo-600"
    },
    {
      title: "Algorithm Design",
      description: "Advanced mathematical models and predictive analytics.",
      icon: Layers,
      color: "from-emerald-500 to-teal-600"
    },
    {
      title: "MLOps",
      description: "Deploying and monitoring models in production environments.",
      icon: Cpu,
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="relative mb-24 text-center md:text-left flex flex-col md:flex-row items-center gap-12">
            <div className="flex-1">
              ...
              >
                <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-bold tracking-widest uppercase mb-8 transition-colors ${isDark ? 'border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-400' : 'border-zinc-200 bg-zinc-50 text-zinc-500'}`}>
                  <Brain className="w-4 h-4" />
                  <span>Domain Specialization</span>
                </div>
                <h1 className="text-5xl md:text-7xl font-bold tracking-tighter mb-8 leading-tight">
                  Machine <span className="bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500 via-purple-500 to-indigo-500">Learning</span>
                </h1>
                <p className={`text-xl ${isDark ? 'text-zinc-400' : 'text-zinc-600'} max-w-2xl leading-relaxed mb-10`}>
                  From deep neural networks to predictive analytics, explore the frontiers of artificial intelligence. BrainstormAI helps you architect intelligent systems that learn, adapt, and scale.
                </p>
                <div className="flex flex-wrap gap-4 justify-center md:justify-start">
                  <button className="relative group">
                    <div className="absolute inset-0 bg-fuchsia-600 rounded-2xl opacity-40 group-hover:opacity-70 blur-md transition-all"></div>
                    <div className="relative flex items-center gap-2 bg-fuchsia-600 px-8 py-4 rounded-2xl font-bold text-white transition-all transform group-hover:-translate-y-1">
            ...
            >
              <div className={`aspect-square max-w-[500px] mx-auto rounded-[3rem] border p-8 flex items-center justify-center relative overflow-hidden ${isDark ? 'bg-white/5 border-white/10' : 'bg-white border-zinc-200 shadow-2xl'}`}>
                <div className="absolute inset-0 bg-gradient-to-br from-fuchsia-500/10 via-transparent to-indigo-500/10" />
                <Brain className={`w-48 h-48 ${isDark ? 'text-fuchsia-400/30' : 'text-fuchsia-500/20'}`} />
                
                {/* Decorative Elements */}
                <motion.div 
                  animate={{ rotate: 360 }}
                  transition={{ duration: 20, repeat: Infinity, ease: "linear" }}
                  className="absolute inset-0 border-2 border-dashed border-fuchsia-500/20 rounded-full m-12"
                />
                <motion.div 
                  animate={{ rotate: -360 }}
                  transition={{ duration: 15, repeat: Infinity, ease: "linear" }}
                  className="absolute inset-0 border border-indigo-500/20 rounded-full m-24"
                />
              </div>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
            {topics.map((topic, index) => (
              <motion.div
                ...
                className={`group p-8 rounded-3xl border transition-all duration-300 ${isDark ? 'bg-white/5 border-white/10 hover:border-fuchsia-500/30 hover:bg-white/[0.08]' : 'bg-white border-zinc-200 hover:border-fuchsia-500/30 shadow-sm'}`}
              >
                <div className={`w-12 h-12 rounded-2xl mb-6 flex items-center justify-center bg-gradient-to-br ${topic.color} text-white shadow-lg shadow-fuchsia-500/20`}>
          ...
          <section className={`rounded-[3rem] p-12 md:p-20 relative overflow-hidden ${isDark ? 'bg-zinc-900 border border-white/5' : 'bg-white border border-zinc-100 shadow-2xl'}`}>
            <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-fuchsia-500/10 to-transparent pointer-events-none" />
            <div className="relative z-10 max-w-3xl">
              <h2 className="text-3xl md:text-5xl font-bold mb-8 tracking-tighter">
                Ready to start your <span className="text-fuchsia-500">Machine Learning</span> journey?
              </h2>
              <p className={`text-lg mb-10 ${isDark ? 'text-zinc-400' : 'text-zinc-500'}`}>
                Our AI engine will analyze your current skill level and project goals to build a custom-tailored curriculum and project blueprint.
              </p>
